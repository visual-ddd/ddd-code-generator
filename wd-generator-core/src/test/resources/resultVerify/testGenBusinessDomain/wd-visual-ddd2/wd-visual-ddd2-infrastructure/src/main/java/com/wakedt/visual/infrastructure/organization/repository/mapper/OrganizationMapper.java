...
public interface OrganizationMapper extends BaseMapper<OrganizationDO> {
    ...
    /** 根据用户查询关联的组织信息列表 */
    OrganizationDO organizationInfoByAccountIdQuery(OrganizationInfoByAccountIdQuery query);

    /** 查询团队详情 */
    OrganizationDO teamQuery(TeamQuery query);

    /** 根据账号查询所属团队信息列表 */
    OrganizationDO teamInfoByAccountIdQuery(TeamInfoByAccountIdQuery query);

    /** 分页查询组织 */
    List<OrganizationDO> organizationPageQuery(OrganizationPageQuery pageQuery);

    /** 分页查询团队 */
    List<OrganizationDO> teamPageQuery(TeamPageQuery pageQuery);

    /** 分页查询团队成员 */
    List<OrganizationDO> teamMemberByTeamIdPageQuery(TeamMemberByTeamIdPageQuery pageQuery);
}